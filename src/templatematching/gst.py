...
class GstRtspProcess:
    ...
    def __init__(self, source_url, width, height, input_queue):
        self.rtsp = GstRtsp(source_url, width, height)
...
class GstRtsp:
    ...
    def __init__(self, source_url, width, height):
    ...
    def timeout(self):
        lprint("[rtsp connection] timeout")
    ...
    def try_to_connect(self):
        ...
        while True:
            ...
            if ret == Gst.StateChangeReturn.SUCCESS:
                lprint("[rtsp connection] success to connect: " + self.source_url)
                break
            else:
                lprint("[rtsp connection] failed to connect to rtsp server: " + self.source_url)
    ...
    def on_message(self, bus, message):
        if message.type == Gst.MessageType.ERROR:
            ...
            lprint(("Error received from element %s: %s" % (
                message.src.get_name(), err)))
            lprint(("Debugging information: %s" % debug))
            self.retry_to_connect()
        elif message.type == Gst.MessageType.EOS:
            lprint("End-Of-Stream reached.")
            self.retry_to_connect()
        elif message.type == Gst.MessageType.STATE_CHANGED:
            if isinstance(message.src, Gst.Pipeline):
                old_state, new_state, pending_state = message.parse_state_changed()
                lprint(("Pipeline state changed from %s to %s." %
