...
class GstRtspSrvProcess:
    ...
    def __init__(self, input_queue, port, width, height):
        self.rtsp = GstRtspSrv(port, width, height)
...
class GstRtspSrv:
    ...
    def __init__(self, port, width, height):
        ...
        lprint(f"[RTSP] ready at {self.url}")

    def client_connected(self, arg1, arg2):
        lprint('[RTSP] next service is connected')

    def on_media_constructed(self, factory, media):
        lprint('[RTSP] media is constructed')
    ...
    def start(self, queue):
        # start rtp server
        lprint(f"[rtp server] start rtp server : {self.url}")
        loop = GLib.MainLoop()
