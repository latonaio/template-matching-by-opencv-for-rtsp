...
class MatchingFromStreamingProcess:
    def __init__(self, input_queue, template_queue, fitness_out_q, image_out_q, wait_q,
        ...
        ms = MatchingFromStreaming(init_image_path, init_template_data, init_template_timestamp)
...
class MatchingFromStreaming:
    def __init__(self, init_image_path, init_template_data, init_template_timestamp):
        self.input_q = None
    ...
    def convert_loop(self, input_q, template_q, fitness_out_q, image_out_q, wait_q):
        ...
        self.image_out_q = image_out_q
        self.wait_q = wait_q
        self.matcher = Matcher(self.init_template_data, self.init_image_path, self.init_template_timestamp)
        ...
        while True:
            ...
            if rcv is None:
                lprint("[MatchingFromStreaming] get stop request")
                self.image_out_q.put(None)
                self.fitness_out_q.put(None)
                break
    ...
    def get_multiple_fitness(self, array):
        ...
        if res:
            pass
            # lprint('get fitness is success', 1)
        else:
            res = res_array = None
            lprint("get fitness is failed", 1)
    ...
    def set_templates(self, templates, template_timestamp):
        ...
        if not res:
            lprint("set template is failed", 1)
        else:
            lprint("set template is success")
        return
